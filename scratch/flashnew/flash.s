...
check_header: ; if the file is smaller than the header, the check will take care of it
    LD  B, AGONFWMAGIC
    LD HL, refstring
    LD DE, buffer
    CALL memorymatch
...
header_match:
    ...
    LD	C, A
    LD	HL, buffer
    LD	DE, 6   ; 2x 3byte integer
    LD	A, mos_fread
    RST.LIL 08h

    ; print vdp firmware size
    LD  HL, vdp_msg
    CALL    printString
    LD  HL, buffer
    LD  HL, (HL)
...
; matches number of bytes in memory:
; B: number of bytes to match > 0
; HL: pointer to (zero-terminated) reference string
; DE: pointer to string to test
; A: 0 - match, 1-255 difference / no match
memorymatch:
    LD  A, (DE)
    CP  (HL)
    JR  NZ, @nomatch
...
checksum:
	.DS 1
matchfailure:
    .ASCIZ "File doesn't contain a valid AGON firmware header\r\n"
matchsuccess:
    .ASCIZ "Valid AGON firmware file\r\n"
vdp_msg:
    .ASCIZ "VDP firmware size: "
mos_msg:
    .ASCIZ "MOS firmware size: "
bytes_msg:
    .ASCIZ " bytes"
buffer:
	.DS BUFFERSIZE
